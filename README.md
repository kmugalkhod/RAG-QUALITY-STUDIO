...
Website sources support preview, collection and reusable source snapshots. Single URLs, explicit URL lists, bounded same-origin crawls and XML sitemaps use origin/path rules, robots.txt, canonical duplicate detection and hard page/depth/byte/time/rate/redirect limits. The backend revalidates DNS and redirects against non-public destinations and never renders or executes fetched HTML. **Collect source & build index** stores immutable HTML revisions plus exact snapshot membership and atomically publishes the initial index. In **Knowledge Base → Indexes → Source snapshots**, inspect the collected pages and build independently named index variants with another saved chunk configuration. Snapshot builds make no Website request, preserve complete snapshot → pipeline version → index lineage, and may still require paid embeddings. Use an exact ready index in an answer pipeline; Experiments identifies when two candidates share the same snapshot.
