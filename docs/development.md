...
Save the validated graph before running it. **Collect source & build index** sends `{"source_input":{"kind":"refresh"}}`. The worker stores immutable raw revisions and a ready source snapshot with exact revision membership, extracts bounded main/article text with heading provenance, classifies items as new, changed, unchanged or removed, and reuses compatible embeddings. Publication advances the destination only after all required content and embeddings succeed; failed or cancelled collection leaves the prior ready index current.

To try different extraction/chunk settings without fetching the Website again, save a new ingestion-pipeline version and choose **Build from source snapshot**. The corresponding request body is `{"source_input":{"kind":"snapshot","source_snapshot_id":"<UUID>"},"destination":{"kind":"new","name":"Precise website"}}`; an existing destination uses `{"kind":"existing","knowledge_set_id":"<UUID>"}`. The server requires a ready snapshot in the same project and an unchanged Website source configuration. It reads the stored artifacts only, while new embeddings may still incur cost. `reuse_stored` remains a temporary legacy request adapter and should not be used by new clients.

Snapshot reads are project-scoped and paginated: `GET /source-snapshots`, `GET /source-snapshots/{id}`, `GET /source-snapshots/{id}/items`, and `GET /source-snapshots/{id}/indexes`. The same ingestion-run endpoints documented above expose progress and per-item results. If a snapshot build is rejected, confirm that the snapshot is ready, belongs to the current project, and that URL/discovery/origin/path/fetch settings match its collection version; collect again when source-selection settings changed. A failed build does not alter the snapshot or the previous ready index.
