...
## Source snapshot storage and recovery

Website source snapshots add durable database membership records while their immutable raw artifacts remain on the document volume. Multiple index families can reference one snapshot, and each build adds processing/chunk/vector history; collection is avoided, but storage and embedding cost are not. Capacity monitoring must therefore include snapshot members, revisions, artifacts, chunks and embeddings rather than counting only current-ready indexes. No snapshot deletion or automatic retention policy exists.

Back up PostgreSQL and the document volume at the same recovery point after active ingestion runs have completed or been cancelled. Restoring only PostgreSQL can leave snapshot members pointing to missing artifacts; restoring only files loses the project, revision, membership and index lineage needed to use them safely. After restore, apply `alembic upgrade head`, verify `/api/ready`, inspect a ready snapshot and its downstream indexes, then perform a bounded retrieval check before resuming schedules. Migration `0018` preserves existing IDs and uses conservative lineage backfill; do not manually assign ambiguous legacy indexes to snapshots.
