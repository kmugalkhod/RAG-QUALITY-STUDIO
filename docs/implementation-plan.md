...
**Site features, app links and assets:** extended the Operate sidebar and search, added a navbar link to Limits and FAQ, and linked Settings, the connection vault and ingestion-run recovery to finished guides. The public-content check now verifies reviewed slugs, screenshot metadata and file validity, diagram titles/descriptions, fenced shell/JSON/Python syntax, secret patterns, example hashes, built assets, app Help targets, search and exclusion of internal plans/QA and test-only API routes. Added four accessible deployment/security/recovery/job-state SVGs with text alternatives, two fresh sanitized missing-provider screenshots at desktop/mobile sizes, and a guarded synthetic restore-rehearsal CLI. All new images and the refreshed T1–T5 screenshots were visually inspected; the T1 upload capture was delayed until loading finished. Fictional fixtures and their hashes remain in the public example pack; no developer document or credential was copied into the site.

**Recovery and security verification:** a source `rag-docs-restore-src` PostgreSQL/pgvector stack stored an encrypted fictional Orchard upload, a ready index and a cited historical query. After stopping its API/worker/dispatcher writers, a PostgreSQL custom archive and document-volume archive were restored into an empty, separately named `rag-docs-restore-dst` stack with the same temporary local keyring. Migrations, health/readiness, worker ping, restored historical evidence, key rewrap and a new successful processing run passed; the latter verified restored raw ciphertext could be decrypted. Both isolated stacks were stopped without `-v`; their named volumes were preserved and only known temporary key/archive/record files removed. A separate isolated PostgreSQL security selection passed 21 tests for security controls, artifact encryption and source connections. This rehearses the documented local recovery path; it does not validate an external KMS/Vault, shared OIDC deployment or a live provider.

**Final checks:** the OpenAPI generator drift check passed for 81 public operations, 162 schemas and 11 artifacts. The docs TypeScript/build, broken-link, 58-route, public-content, search and asset check passed. Playwright passed T1–T5 again on the isolated PostgreSQL/pgvector deterministic-provider stack through canonical Vite `http://127.0.0.1:5273` (5/5), the separate missing-provider Settings journey (1/1), and documentation desktop/720px/390px navigation, search, keyboard, diagram and image checks (4/4). Frontend typecheck, lint, all 121 Vitest tests and production build passed; the restore CLI passed Ruff lint/format and Python compilation; `git diff --check` passed. The corrected deployment diagram was rendered and reviewed. No paid model call, live Website/S3/Notion/Confluence account, external OCR/KMS/Vault, or shared OIDC/TLS deployment was accepted. No documentation was published or deployed. The next action is a review of this local phase boundary; any live integration gate or publication needs separate authorization.

## User documentation website — reviewable phase 3 boundary (2026-09-26)

**Phase 3 complete for local review, not published:** rechecked the current retrieval, Playground/history, dataset and experiment UI and backend contracts. Added eight authored task pages: Retrieval settings, Playground, Query history and failure, Datasets, Run experiments, Metrics and costs, Compare results, and Interpret and improve. T4 now walks through two saved answer versions differing only in Top k against one ready index and a reviewed fictional two-question dataset. Added an authored API reference landing page and workflow recipes, nine generated endpoint-group pages and one generated schema-model catalog. The site has 51 public pages and stable routes. Each new page states the local outcome, prerequisites, current labels or tested commands, expected and failure states, related guides, and the boundary between deterministic verification and configured live services.

**Site, app links and fixtures:** grouped operation contracts come from canonical `app.openapi()` with 69 paths, 81 operations and 162 schemas; generation has a drift check, and the public OpenAPI excludes three test-only provider routes. Human-authored API pages explain authentication, project versions, errors, asynchronous jobs and costs. Local search indexes task and operation pages while leaving the large schema catalog navigable without dominating results. Added contextual guides in Playground, query history, dataset import, experiment setup, candidate summaries and per-question evidence. Added reviewed fictional `orchard.txt` and `orchard-reviewed.csv` with recorded SHA-256 hashes, accessible comparison and API-workflow diagrams with text alternatives, and four fresh sanitized T4 screenshots. No real source material, credentials or project IDs appear in public assets. Corrected OpenAPI response media contracts for CSV exports and PNG thumbnails, with a regression assertion.

**Verification:** frontend typecheck, lint, all 121 Vitest tests and production build passed. The isolated PostgreSQL/pgvector backend selection passed 55 API, experiment and retrieval tests; Ruff lint/format passed. The running fixture-backed backend returned a valid `/openapi.json`; after removing test-only routes it matched the checked public schema exactly, including `text/csv` and `image/png` responses. API generation drift check covered 81 operations, 162 schemas and 11 artifacts. A guarded API recipe check exercised one read example per endpoint group plus fictional project → upload → processing → ready index → cited query → dataset preview/import. Docusaurus TypeScript/build, public-content, broken-link, route, search and app-link checks passed for all 51 pages/routes after the final API diagram; the diagram was visually reviewed. Playwright passed T4 (two candidates, four persisted question outcomes, unavailable metrics, paired sample count, CSV export and 390px layout) and all three docs-site navigation/search/keyboard/desktop/720px/390px journeys. The `SnapshotPanel` test briefly timed out under concurrent browser and build load, then passed both alone and in the full 121-test run.

**Live gates and next slice:** no paid OpenRouter embedding/chat/evaluator call, live Website/S3/Notion/Confluence account, external OCR/KMS/Vault, or shared OIDC deployment was tested. Fixed test-judge scores are bookkeeping evidence only, not a quality comparison. No site publishing or deployment was performed. Phase 4 remains: Deploy locally and beyond, Security and permissions, Backup and restore, Operations and monitoring, Troubleshooting, Limits and FAQ; then an operator recovery rehearsal, release review, and documentation maintenance checks. The Phase 3 isolated services are stopped without deleting named volumes after the final checks; the canonical Vite frontend is restored to its ordinary local proxy.

## User documentation website — reviewable phase 2 boundary (2026-09-26)

**Phase 2 complete for local review, not published:** rechecked source adapters, schema-v2 ingestion forms, preview/run responses and source-history behavior against the dated documentation plan. Added 13 authored public pages: Source connections, Choose a source, Website, Amazon S3, Notion and Confluence sources, Extraction and OCR, Cleaning/language/sensitive data, Chunking and embedding, Preview processing, Run and inspect ingestion, Quality findings and Source history/snapshot reuse. The site now has 31 navigable public pages with local full-text search and stable slugs. T3 documents a fixed Website snapshot reused for a second index; T5 documents a strict quality failure, repaired source and redacted publication. Both state the deterministic local gate and the separate external-account/provider gates. Guarded connection, role, retention, SSRF and failed-publication notes are embedded in the relevant guides; internal plans and QA records remain outside the public build.

**App links, diagrams and fixtures:** added contextual links from the current source-kind heading, each ingestion stage, connection vault, processing preview, quality summary, run inspector and Source history. Added reviewed fictional corrupt/repaired text files with SHA-256 hashes, a source-lineage diagram and quality-repair diagram with text alternatives, and four fresh component screenshots of T3/T5 outcomes. The screenshot manifest identifies the isolated test transport and sanitized crops. Auditing the S3 guide exposed a contract mismatch: the editor and adapter offered nine file types while `S3Config.allowed_file_types` allowed only two selections. The schema now accepts all nine supported types with a focused bound regression test.

**Verification:** docs TypeScript/build, broken-link/route/static-asset/public-content/search/app-link checks passed for 31 pages and 31 routes. The running isolated PostgreSQL/pgvector, Redis and deterministic test-provider stack returned `/api/ready` 200 and `/openapi.json` 200 with 72 paths, 84 operations and 162 schemas. Focused backend contracts passed (15 tests) with Ruff lint/format; frontend typecheck, lint, 121 Vitest tests and production build passed. Playwright completed T1/T2, T3 Website snapshot → saved second pipeline version → index variant, and T5 strict failure → repaired/redacted publication. Docs navigation/search, synthetic downloads, alt text and desktop/720px/390px responsive/keyboard checks passed after fixing a hidden search-measurement element that caused 720px overflow with a long query. All new screenshots were visually reviewed for fictional content and absence of credentials, project IDs or real source data. The repository-wide frontend Prettier check still reports two untouched baseline files (`src/lib/api.ts` and `tests/features/ingestion-pipelines/IngestionResults.test.tsx`); changed documentation/UI files pass focused Prettier.

**Live gates and next slice:** no paid OpenRouter embedding/chat/evaluator call, real Website fetch, S3/Notion/Confluence account, external OCR/KMS/Vault, or shared OIDC deployment was tested. The external connector guides therefore remain conditional. No site publishing or deployment was performed. Phase 3 is next: retrieval and Playground/history guides; datasets, experiments, metrics/costs, comparison and interpretation; T4; and generated grouped endpoint contracts with authored API workflow recipes. Phase 4 still covers deployment, security, backup/restore, operations, troubleshooting, limits/FAQ and release maintenance.

## User documentation website — reviewable phase 1 boundary (2026-09-26)

**Phase 0 complete:** rechecked the current app routes, labels, test workflows, provider settings and connector gates against the dated `docs/documentation-plan.md`. Repaired schema-v2 ingestion OpenAPI generation by keeping the closed node union and inner chunk `algorithm` discriminator without an invalid nested outer discriminator. Added regression coverage for `app.openapi()`, `/openapi.json`, operation request/response contracts and component references. Corrected stale Settings, Overview, Knowledge Base empty-state and chunk-provenance copy; updated affected browser selectors. The isolated API returned HTTP 200 for `/openapi.json` (72 paths, 84 operations, 162 component schemas) and `/api/ready`.

**Phase 1 complete for local review, not published:** added a Docusaurus 3/TypeScript site in `docs/site/` with 18 authored public pages: Choose your path, Install locally, Configure model providers, First RAG workflow; How the workflow works, Glossary, Projects and workspace, Versions and provenance; Knowledge Base, Upload and manage documents, Prepare uploaded content, Collections and index versions; Build an Existing Files ingestion pipeline; Save an answer pipeline, Ask questions and inspect evidence; API overview, API patterns and versioned jobs; Current capability boundaries. T1 and T2 are complete within their task guides. Every page has a stable slug, `verified_against` field, outcome, prerequisite/gate, current labels or tested commands, expected result, retry guidance and related links. The site has an audience landing page, grouped sidebar, breadcrumbs, page TOC, previous/next navigation, current-commit label, responsive theme and working account-free local search. A public-content check verifies built routes, app Help targets, search entries, the sample download and exclusion of internal plans/QA/secrets.

**App links and fixtures:** Help & documentation in the project sidebar; contextual links from document upload, collection publication, Settings, Existing Files editor, answer graph validation and Playground evidence. All target completed public routes; URL construction carries no project/source/question data. The fictional `harbor-desk.txt`, workflow SVG and nine newly captured, visually reviewed synthetic screenshots are public assets; `docs/site/screenshot-manifest.json` records date, base commit, viewport and isolated test scenario without publishing QA records. `README.md` points to the local docs preview and corrects prominent historical labels.

**Verification passed:** backend focused `pytest tests/test_api.py tests/test_ingestion_contracts.py -q` (28 passed, 7 database-dependent skips in the local process), Ruff lint and formatting; frontend typecheck, structure/ESLint, 121 Vitest tests and production build; docs clean `npm ci && npm run check` (TypeScript, build/broken links, 18 routes, public/search/app-link audit). On a separate `rag-docs-e2e` PostgreSQL/pgvector, Redis, worker and deterministic test-provider stack, Playwright completed T1 upload → processing → ready collection → saved answer → cited evidence, T2 Existing Files preview → saved version → successful index, and the project Settings/navigation/isolation journey. Docs browser checks passed desktop, 720px and 390px widths, no horizontal overflow, search result navigation, synthetic download, image availability, alt text and keyboard search/mobile navigation. The application was served by canonical Vite at `http://127.0.0.1:5273`; docs preview was separate at `http://127.0.0.1:3000`. The isolated Compose services were stopped without `-v` after verification, leaving their volumes intact, and Vite was restored to the ordinary local backend proxy.

**Live gates not run:** no paid OpenRouter embedding/chat/evaluator call; no real Website, S3, Notion or Confluence account; no external OCR/KMS/Vault or shared OIDC deployment acceptance. The Docusaurus package still reports 19 moderate transitive npm advisories after overriding a high-severity build dependency; review these before any deployment. No site publishing or deployment was performed.

**Remaining after this boundary:** Phase 2 through Phase 4 were outstanding when phase 1 closed; the current status and next slice are recorded above.
