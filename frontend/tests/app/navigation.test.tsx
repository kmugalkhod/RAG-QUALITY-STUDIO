...
import { parseRoute, useRoute, useUnsavedChanges } from '../../src/app/navigation';
const id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';
beforeEach(() => {
  window.history.replaceState(null, '', '/');
});
it('parses project, page, editor and saved version links', () => {
  expect(parseRoute(`#/projects/${id}/pipelines/new`)).toMatchObject({
    projectId: id,
    page: 'pipelines',
    detail: 'new',
  });
...
it('keeps the current route when a dirty editor navigation is rejected', async () => {
  ...
  render(<Guard />);
  act(() => {
    window.location.hash = `/projects/${id}/overview`;
  });
