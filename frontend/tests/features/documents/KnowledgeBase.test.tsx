...
beforeEach(() => {
  ...
  vi.mocked(indexApi.getEmbeddingSettings).mockResolvedValue({
    configured: false,
    config: null,
    error: 'Configure server-side embeddings.',
  });
});
import { render, screen, waitFor, within, cleanup } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { afterEach, beforeEach, expect, test, vi } from 'vitest';
import { KnowledgeBase } from '../../../src/features/documents/KnowledgeBase';
import * as api from '../../../src/features/documents/api';
import * as apiModel from '../../../src/features/documents/model';
vi.mock('../../../src/features/documents/api');
const doc: apiModel.Document = {
  id: 'd1',
  filename: 'source.txt',
  size_bytes: 10,
  content_hash: 'abc',
...
beforeEach(() => {
  ...
  vi.mocked(api.getUploadSettings).mockResolvedValue({ max_upload_bytes: 1024 });
  vi.mocked(api.listDocuments).mockResolvedValue(page([]));
  vi.mocked(api.listProcessingRuns).mockResolvedValue(page([]));
});
afterEach(() => {
  cleanup();
  vi.resetAllMocks();
...
test('uploads and validates chunk settings', async () => {
  ...
  expect(api.startProcessingRun).not.toHaveBeenCalled();
});

test('shows upload progress and storage failure', async () => {
  let reject!: (reason: Error) => void;
  vi.mocked(api.uploadDocument).mockReturnValue(
    new Promise((_, fail) => {
      reject = fail;
    }),
  );
...
test('loads saved chunks and retries a failed request', async () => {
  ...
  vi.mocked(api.listDocumentChunks)
    .mockRejectedValueOnce(new Error('Connection interrupted.'))
    .mockResolvedValue(
      page([{ ordinal: 0, page_number: 2, start_char: 0, end_char: 4, text: 'abcd' }]),
    );
  render(<KnowledgeBase projectId="p1" />);
  await userEvent.click(screen.getByRole('button', { name: 'Add document' }));
...
test('shows processing progress and cancels a queued run', async () => {
  ...
  vi.mocked(api.listProcessingRuns).mockResolvedValue(page([queued]));
...
test('retains upload settings errors when the document list succeeds and offers retry', async () => {
  vi.mocked(api.getUploadSettings)
    .mockRejectedValueOnce(new Error('Settings unavailable.'))
    .mockResolvedValue({ max_upload_bytes: 1024 });
  render(<KnowledgeBase projectId="p1" />);
  await userEvent.click(screen.getByRole('button', { name: 'Add document' }));
