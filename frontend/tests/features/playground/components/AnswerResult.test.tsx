...
import {
  RunResult,
  RunInspector,
} from '../../../../src/features/playground/components/AnswerResult';
...
const run: QueryRun = {
  ...
  snapshot: {
    ...
    citations: {
      ...
      invalid: ['S99'],
      missing: false,
      semantics: 'Reference membership only.',
    },
  },
};
...
describe('query result', () => {
  ...
  it('shows failed runs and preserved evidence', () => {
    render(
      <>
        <RunResult
          run={{ ...run, status: 'failed', answer: null, error: 'OpenRouter timed out.' }}
        />
        <RunInspector run={run} mode="sources" />
      </>,
    );
    ...
    render(
      <RunResult
        run={{
          ...
          answer: 'INSUFFICIENT_EVIDENCE: No answer.',
        }}
      />,
    );
