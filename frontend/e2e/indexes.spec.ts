...
test('indexes and retrieves persisted source evidence using the isolated provider fixture', async ({ page, request }) => {
  ...
  await page.reload();
  await expect(page.getByRole('button', { name: 'Document sets', exact: true })).toHaveAttribute('aria-current', 'page');
  ...
  await page.getByRole('heading', { name: 'Prepare documents for questions' }).scrollIntoViewIfNeeded();
  await page.getByRole('region', { name: 'Prepare documents for questions' }).screenshot({ path: 'test-results/index-desktop.png' });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.getByRole('heading', { name: 'Prepare documents for questions' }).scrollIntoViewIfNeeded();
  await page.getByRole('region', { name: 'Prepare documents for questions' }).screenshot({ path: 'test-results/index-mobile.png' });
