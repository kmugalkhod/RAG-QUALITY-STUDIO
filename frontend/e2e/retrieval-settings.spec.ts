...
test('legacy pipeline upgrades to hybrid, persists all six settings and tests keyword retrieval', async ({
  page,
  request,
}) => {
  test.skip(
  ...
  const project = await (
    await request.post('/api/projects', { data: { name: `Retrieval settings ${Date.now()}` } })
  ).json();
  const root = `/api/projects/${project.id}`;
  const doc = await (
    await request.post(`${root}/documents`, {
      multipart: {
        file: {
          name: 'orchard.txt',
          mimeType: 'text/plain',
  ...
  const processing = await (
    await request.post(`${root}/documents/${doc.id}/runs`, {
      data: { chunk_size: 500, overlap: 0 },
    })
  ).json();
  await expect
    .poll(
      async () =>
        (await (await request.get(`${root}/documents/${doc.id}/runs/${processing.id}`)).json())
          .status,
      { timeout: 40000 },
    )
    .toBe('succeeded');
  ...
  await expect
    .poll(async () => (await (await request.get(`${root}/indexes/${index.id}`)).json()).status, {
      timeout: 40000,
    })
    .toBe('succeeded');
  const kinds = ['question', 'retriever', 'prompt', 'llm', 'answer'];
  const original = await (
    await request.post(`${root}/pipelines`, {
      data: {
        ...
        execution: {
          ...
          nodes: [
            ...
            { id: 'retriever', type: 'retriever', index_id: index.id, top_k: 5 },
            { id: 'prompt', type: 'prompt', template: 'Answer {question} using {context}' },
            { id: 'llm', type: 'llm', model: 'test/chat', max_tokens: 512, temperature: 0 },
            { id: 'answer', type: 'answer' },
          ],
          edges: kinds.slice(1).map((kind, i) => ({ source: kinds[i], target: kind })),
        },
        layout: {
          positions: Object.fromEntries(kinds.map((kind, i) => [kind, { x: 80, y: 40 + i * 116 }])),
        },
      },
    })
  ).json();
  await page.goto(
    `/#/projects/${project.id}/pipelines/${original.pipeline_id}?version=${original.id}`,
  );
  ...
  const previewResponse = page.waitForResponse(
    (r) => r.url().endsWith('/preview-runs') && r.request().method() === 'POST',
    { timeout: 15000 },
  );
  await page.getByRole('button', { name: 'Run pipeline test', exact: true }).click();
  const preview = await (await previewResponse).json();
  await expect
    .poll(async () => (await (await request.get(`${root}/query-runs/${preview.id}`)).json()).status)
    .toBe('succeeded');
  ...
  expect(
    (
      await (
        await request.get(`${root}/pipelines/${original.pipeline_id}/versions/${original.id}`)
      ).json()
    ).execution,
  ).toEqual(original.execution);
  await page.getByRole('button', { name: 'Retrieval test', exact: true }).click();
  await page
    .getByRole('combobox', { name: 'Documents to search', exact: true })
  ...
  await expect(page.getByRole('region', { name: 'Retrieval test results' })).toContainText(
    'Keyword score',
  );
});
