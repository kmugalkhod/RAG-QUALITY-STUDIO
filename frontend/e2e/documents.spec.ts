...
test('shows parsing failure and allows a new processing version', async ({ page, request }) => {
  const project = (await (
    await request.post('/api/projects', { data: { name: `Failure verification ${Date.now()}` } })
  ).json()) as { id: string };
  ...
  await page.getByRole('button', { name: 'Start processing', exact: true }).click();
  await expect(
    page.getByText('PDF could not be parsed. Upload a valid, unencrypted text-based PDF.').last(),
...
test('processes a text PDF and displays its page provenance', async ({ page, request }) => {
  const project = (await (
    await request.post('/api/projects', { data: { name: `PDF verification ${Date.now()}` } })
  ).json()) as { id: string };
  await page.goto(`/#/projects/${project.id}`);
  await page.getByRole('button', { name: 'Add document', exact: true }).click();
