...
test('pipeline create configure save reopen run evidence and immutable versions', async ({ page, request }) => {
  ...
  await page.getByRole('link', { name: 'Open pipeline editor' }).click();
  ...
  await page.reload();
  await page.getByLabel('Open pipeline', { exact: true }).selectOption({ label: 'Orchard answers' });
  await expect(page.getByText('Saved version 1', { exact: true })).toBeVisible();
  await page.getByLabel('Selected node').selectOption('prompt');
  ...
  await expect(page.getByRole('button', { name: 'Run pipeline', exact: true })).toBeDisabled();
  await page.getByRole('button', { name: 'Save version', exact: true }).click();
  ...
  await page.getByRole('button', { name: /What does the orchard grow\? · version 1/ }).click();
  ...
  await page.locator('.pipeline-editor').screenshot({ path: 'test-results/pipeline-desktop.png' });
