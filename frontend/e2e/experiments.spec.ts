...
test('import reviewed dataset, compare two versions and inspect evidence', async ({
  page,
  request,
}) => {
  test.skip(
  ...
  const nodes = [
    { id: 'q', type: 'question' },
    { id: 'r', type: 'retriever', index_id: index.id, top_k: 1 },
    { id: 'p', type: 'prompt', template: '{question} {context}' },
    { id: 'l', type: 'llm', model: 'test/chat', temperature: 0, max_tokens: 512 },
    { id: 'a', type: 'answer' },
  ];
  const payload = {
    name: 'Orchard comparison',
    execution: {
      schema_version: 1,
      nodes,
      edges: nodes.slice(1).map((n, i) => ({ source: nodes[i].id, target: n.id })),
    },
    layout: { positions: Object.fromEntries(nodes.map((n, i) => [n.id, { x: 0, y: i * 120 }])) },
  };
  const first = await (
    await request.post(`/api/projects/${project.id}/pipelines`, { data: payload })
  ).json();
  nodes[1].top_k = 2;
  const second = await (
    await request.post(`/api/projects/${project.id}/pipelines/${first.pipeline_id}/versions`, {
      data: payload,
    })
  ).json();
  ...
  await expect(page.getByRole('combobox', { name: 'Candidate A', exact: true })).toHaveValue(
    first.id,
  );
  await expect(
    page.getByRole('combobox', { name: 'Candidate B (optional)', exact: true }),
  ...
  await expect(
    evidence.getByText('The orchard grows apples. Harvest begins in September.', { exact: true }),
  ).toHaveCount(2);
