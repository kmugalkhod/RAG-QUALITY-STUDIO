...
export function App() {
  ...
  const pipelineId = /^#\/projects\/([a-f0-9-]{36})\/pipelines$/.exec(hash)?.[1];
  ...
  return <div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><span>Workspace / <strong>{pipelineId ? 'Pipeline editor' : playgroundId ? 'RAG playground' : projectId ? 'Knowledge Base' : 'Projects'}</strong></span><a href="http://localhost:8000/docs" target="_blank" rel="noreferrer">API reference <ArrowUpRight size={14}/></a></header><main id="main"><>{pipelineId ? <PipelineEditor key={pipelineId} projectId={pipelineId}/> : playgroundId ? <Playground key={playgroundId} projectId={playgroundId}/> : projectId ? <KnowledgeBase key={projectId} projectId={projectId}/> : <ProjectsPage />}</></main><footer>RAG Quality Studio <span>Saved pipelines · Milestone 4</span></footer></div>
  </div>;
}
