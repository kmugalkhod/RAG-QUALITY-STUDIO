...
export function App() {
  ...
  useEffect(() => {
    if (projectId && (page === 'knowledge-base' || page === 'playground')) sessionStorage.setItem(`${page}:${projectId}`, `#/projects/${projectId}/${page}${queryString ? `?${queryString}` : ''}`);
  ...
  return <div className="app-shell linear-workspace">
    ...
    <aside className={`sidebar ${mobile ? 'sidebar-open' : ''}`} onKeyDown={e => { if (e.key === 'Escape') { setMobile(false); document.querySelector<HTMLButtonElement>('.mobile-toggle')?.focus(); } }}>
      ...
      <div id="workspace-navigation" className="sidebar-content">
        <label className="project-switcher">Project<select aria-label="Switch project" value={projectId || ''} onChange={e => { const destination = e.target.value;
          window.location.hash = destination ? ((page === 'knowledge-base' || page === 'playground') && sessionStorage.getItem(`${page}:${destination}`)) || `/projects/${destination}/${pages.some(p => p[0] === page) ? page : 'overview'}` : '/'; }}><option value="">All projects</option>{current && !projects.some(p => p.id === current.id) && <option value={current.id}>{current.name}</option>}{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
        <nav aria-label="Project navigation">{projectId ? pages.map(([path, label, Icon]) => <a key={path} href={(path === 'playground' || path === 'knowledge-base') ? sessionStorage.getItem(`${path}:${projectId}`) || `${base}/${path}` : `${base}/${path}`} className={page === path ? 'nav-active' : ''} aria-current={page === path ? 'page' : undefined}><Icon size={18}/>{label}</a>) : <a href="#/" className="nav-active" aria-current="page"><Folder size={18}/>Projects</a>}</nav>
    ...
    <div className="workspace"><header className="topbar"><span>{projectId ? current?.name || 'Loading project…' : 'Workspace'} <span aria-hidden="true">/</span> <strong>{title}{page === 'pipelines' && detail ? ' / Editor' : ''}</strong></span></header><main id="main" tabIndex={-1} className={page === 'playground' ? 'playground-main' : page === 'knowledge-base' ? 'knowledge-main' : page === 'pipelines' && detail ? 'editor-main' : ''}>
      {projectId && !current ? error ? <div role="alert"><h1>Project unavailable</h1><p>{error}</p><button onClick={() => setRevision(v => v + 1)}>Retry</button></div> : <p role="status">Loading project…</p> : <div key={`${projectId}/${page}/${detail || ''}`} className={`page-content ${page}-page`}>{page === 'projects' ? <ProjectsPage onCreated={() => setRevision(v => v + 1)}/> : page === 'overview' ? <ProjectSummary projectId={projectId!}/> : page === 'experiments' ? <ExperimentsPage projectId={projectId!} experimentId={detail}/> : page === 'settings' ? <ProjectSummary projectId={projectId!} configuration/> : page === 'knowledge-base' ? <KnowledgeBase projectId={projectId!} documentId={route.query.get('document') || ''}/> : page === 'pipelines' ? detail ? <PipelineEditor projectId={projectId!} pipelineId={detail} versionId={route.query.get('version') || ''}/> : <PipelinesPage projectId={projectId!}/> : page === 'playground' ? <Playground projectId={projectId!} pipelineId={route.query.get('pipeline') || ''} versionId={route.query.get('version') || ''} readyIndexId={route.query.get('index') || ''} retrievalCount={route.query.get('top_k') || '5'} testMode={route.query.get('mode') || 'pipeline'}/> : <><h1>Page not found</h1><a href="#/">Return to projects</a></>}</div>}
    </main></div>
  </div>;
}
