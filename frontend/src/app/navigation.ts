...
export function parseRoute(hash: string) {
  ...
  return {
    projectId: match?.[1],
    page: match ? match[2] || 'knowledge-base' : path && path !== '/' ? 'not-found' : 'projects',
    detail: match?.[3],
...
export function useRoute() {
  ...
  useEffect(() => {
    const update = () => {
      if (blocker && window.location.href !== blocker.href && !blocker.allow()) {
        window.history.replaceState(null, '', blocker.href);
      }
...
export function useUnsavedChanges(dirty: boolean) {
  useEffect(() => {
    ...
    const guard = {
      ...
      allow: () => approved || window.confirm('Discard unsaved pipeline changes?'),
    };
    blocker = guard;
