...
test('shows missing server configuration and keeps paid action disabled', async () => {
  ...
  expect(screen.getByRole('button', { name: 'Index documents' })).toBeDisabled();
...
test('creates an index, shows progress and cancels', async () => {
  ...
  render(<IndexPanel projectId="p1"/>);
  await waitFor(() => expect(screen.getByRole('button', { name: 'Index documents' })).toBeEnabled());
  await userEvent.click(screen.getByRole('button', { name: 'Index documents' }));
  expect(await screen.findByText(/Index version 1 queued with 3 chunks/)).toBeVisible();
  expect(await screen.findByRole('progressbar')).toHaveAttribute('value', '1');
  expect(screen.getByRole('button', { name: 'Index documents' })).toBeDisabled();
  await userEvent.click(screen.getByRole('button', { name: 'Cancel index 1' }));
...
test('retrieves from explicit version and exposes source evidence and distance', async () => {
  ...
  render(<IndexPanel projectId="p1"/>);
  await userEvent.click(await screen.findByRole('button', { name: 'Use index 1' }));
  ...
  expect(screen.getByText(/1 results from index version 1/)).toBeVisible();
});
...
test('validates queries and top k; retry preserves error until a new action', async () => {
  ...
  await userEvent.click(screen.getByRole('button', { name: 'Test retrieval' }));
  expect(await screen.findByText('No matching chunks in this index.')).toBeVisible();
});
...
test('loading errors can be retried; index failures are visible', async () => {
  ...
  expect(screen.queryByRole('button', { name: 'Use index 1' })).toBeNull();
});
