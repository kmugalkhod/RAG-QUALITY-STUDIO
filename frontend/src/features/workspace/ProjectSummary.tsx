...
export function ProjectSummary({ projectId, configuration = false }: { projectId: string; configuration?: boolean }) {
  ...
  const next = !data.documents.length ? ['Add your first source', 'Upload a PDF or TXT document to begin.', 'knowledge-base', 'Upload documents'] : !processed.length ? ['Prepare your documents', 'Process uploaded text into inspectable chunks.', 'knowledge-base', 'Process documents'] : !ready.length ? ['Make your sources searchable', indexing.length ? 'Documents are being prepared for questions. Check progress in the Knowledge Base.' : 'Prepare a document set from your processed documents.', 'knowledge-base', 'View document preparation'] : !data.pipelines.length ? ['Configure your first pipeline', 'Your documents are ready for questions. Choose how answers should be written.', 'pipelines/new', 'Create pipeline'] : ['Ask your sources a question', 'Choose a saved pipeline and inspect the evidence behind its answer.', 'playground', 'Open Playground'];
  return <><div className="page-heading"><div><h1>Overview</h1><p>{data.project.description || 'Sources and saved configurations for this project.'}</p></div><Button asChild><a href={`#/projects/${projectId}/${next[2]}`}>{next[3]}<ArrowRight size={14}/></a></Button></div>
    <div className="overview-layout"><div className="overview-content"><section className="project-readiness"><h2>{next[0]}</h2><p>{next[1]}</p><div className="readiness-line"><span>{processed.length} processed documents</span><span>{ready.length} prepared document sets</span><span>{data.pipelines.length} saved pipelines</span></div></section>
