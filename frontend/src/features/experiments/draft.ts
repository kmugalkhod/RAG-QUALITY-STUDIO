...
export function readDraft(project: string): ExperimentDraft {
  try {
    ...
    const string = (name: string, max: number) =>
      typeof d[name] === 'string' ? d[name].slice(0, max) : '';
    const a = string('a', 36);
    const b = string('b', 36);
    return {
      name: string('name', 120),
      dataset: string('dataset', 36),
      a,
      b: b === a ? '' : b,
      metrics: Array.isArray(d.metrics)
        ? metrics.filter((m) => d.metrics instanceof Array && d.metrics.includes(m))
        : [...metrics],
    };
  } catch {
    return emptyDraft();
...
export function saveDraft(project: string, draft: ExperimentDraft): boolean {
  try {
    sessionStorage.setItem(key(project), JSON.stringify(draft));
    return true;
  } catch {
    return false;
  }
