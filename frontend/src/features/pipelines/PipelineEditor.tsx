...
function WorkflowNode({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  const detail = c.type === 'retriever' ? `Top k ${c.top_k} · ${c.index_id ? 'Index selected' : 'Choose an index'}` : c.type === 'llm' ? c.model || 'Choose a model' : c.type === 'prompt' ? 'Answer with evidence' : c.type === 'question' ? 'User input · single turn' : 'Response + citations';
  ...
  return <div className="editor-workspace">
    ...
    <fieldset className="pipeline-fields" disabled={busy || !!running || loading}>
      <div className="pipeline-toolbar"><label>Pipeline name<input value={name} maxLength={120} onChange={e => setName(e.target.value)}/></label>
        ...
        <Button variant={saved && !dirty ? 'default' : 'outline'} disabled={!saved || dirty || busy} onClick={() => { if (saved) window.location.hash = `/projects/${projectId}/playground?pipeline=${saved.pipeline_id}&version=${saved.id}`; }}><Play size={15}/>Test pipeline</Button>
