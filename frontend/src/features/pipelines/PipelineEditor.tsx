...
function WorkflowNode({ data, selected }: NodeProps<FlowNode>) {
  ...
  return <div className={`workflow-node${selected ? ' workflow-selected' : ''}`}>
    ...
    <header><strong><Icon size={15}/>{data.label}</strong><span>{descriptions[c.type]}</span></header>
...
export function PipelineEditor({ projectId, pipelineId }: { projectId: string; pipelineId: string }) {
  ...
  const open = useCallback((v: api.Version) => {
    ...
    void flowRef.current?.setCenter(position.x + 110, position.y + 70, { zoom: 0.9 });
  ...
  useEffect(() => {
    ...
    void Promise.all([api.options(projectId), api.allPages(o => listIndexes(projectId, o)), pipelineId === 'new' ? Promise.resolve([]) : api.allPages(o => api.versions(projectId, pipelineId, o))]).then(([o, ix, vs]) => {
      ...
      if (pipelineId !== 'new') { if (!vs.length) throw new Error('No saved versions found.'); setVersions(vs); open(vs[0]); }
      else { const configs = api.order.map(k => ({ id: k, type: k, ...(k === 'retriever' ? { index_id: '', top_k: 5 } : k === 'prompt' ? { template: o.template } : k === 'llm' ? { model: o.models[0] || '', max_tokens: o.max_tokens, temperature: 0 } : {}) })); setNodes(configs.map((n, i) => flowNode(n, { x: i * 260, y: 40 }))); setEdges(api.order.slice(1).map((k, i) => ({ id: `${api.order[i]}-${k}`, source: api.order[i], target: k }))); }
    }).catch(e => { if (!disposed) setError(errorText(e)); }).finally(() => { if (!disposed) setLoading(false); });
    return () => { disposed = true; };
  }, [projectId, pipelineId, refresh, open, setNodes, setEdges]);
  ...
  return <div className="editor-workspace">
    ...
    <div className="editor-title"><h1>Pipeline editor</h1><span>Configure → Save → Test in Playground</span></div>
    ...
    <fieldset className="pipeline-fields" disabled={busy || !!running || loading}>
      <div className="pipeline-toolbar"><label>Pipeline name<input value={name} maxLength={120} onChange={e => setName(e.target.value)}/></label>
        ...
        <Button variant={dirty ? 'default' : 'outline'} aria-describedby="save-guidance" disabled={saveReasons.length > 0} onClick={() => void action(async () => { const v = await api.save(projectId, draft, saved?.pipeline_id); open(v); setVersions(await api.allPages(o => api.versions(projectId, v.pipeline_id, o))); window.history.replaceState(null, '', `#/projects/${projectId}/pipelines/${v.pipeline_id}`); })}>Save version</Button>
        <Button variant={saved && !dirty ? 'default' : 'outline'} disabled={!saved || dirty || busy} onClick={() => { if (saved) window.location.hash = `/projects/${projectId}/playground?pipeline=${saved.pipeline_id}&version=${saved.id}`; }}>Test pipeline</Button>
        <Button variant="outline" disabled={!saved || dirty} onClick={() => void action(async () => { const v = await api.save(projectId, { ...draft, name: `${name.slice(0, 113)} (copy)` }); open(v); setVersions([v]); window.history.replaceState(null, '', `#/projects/${projectId}/pipelines/${v.pipeline_id}`); })}>Duplicate pipeline</Button>
