...
export function RunInspector({ run, mode, sourceLabel = '', focusRequest = 0 }: { run: api.QueryRun; mode: 'sources' | 'details'; sourceLabel?: string; focusRequest?: number }) {
  ...
  return <section className="answer-inspector" aria-label={mode === 'sources' ? 'Supporting evidence' : 'Answer details'}>
    ...
    {s.evidence.map(e => <article key={e.label} id={`evidence-${e.label}`} tabIndex={-1}><h3>[{e.label}] {e.filename}</h3><p className="field-hint">{e.page_number ? `Page ${e.page_number} · ` : ''}Passage {e.ordinal + 1} · Rank {e.rank}</p><p className="rag-source-text">{e.text}</p><details className="source-metadata"><summary>Source metadata</summary><p>Processing version {e.processing_version} · Cosine distance {e.cosine_distance.toFixed(4)}</p><p>Distance is not confidence; lower is closer.</p></details></article>)}
