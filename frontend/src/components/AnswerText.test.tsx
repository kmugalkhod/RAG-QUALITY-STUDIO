...
test('formats paragraphs, emphasis, lists and code while retaining citation controls', async () => {
  const select = vi.fn();
  const { container } = render(<AnswerText text={'**30 days** [S1]\n\n- Receipt required\n- *Unused* sensor\n\n1. Contact support\n\n```\n<script>unsafe()</script>\n```'} citations={['S1']} onCitation={select}/>);
...
test('keeps HTML, links and invalid source labels inert', () => {
  const { container } = render(<AnswerText text={'<img src=x onerror=alert(1)> [S99] [click](javascript:alert(1)) `**literal**`'} citations={['S1']} onCitation={vi.fn()}/>);
